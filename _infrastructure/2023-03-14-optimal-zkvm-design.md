...
*Our vision:* We want to make it practical to compile conventional code to a zkVM with reasonable performance. No zkVM today is optimized for this goal. We desire a VM with neither registers nor a stack, and one that has a small field size with low-degree constraints. We also want efficient compilation from LLVM IR to the set of VM instructions. 

Github repository: [delendum-xyz/valida](https://github.com/delendum-xyz/valida)

### Table of contents

- [zkVM Design](#zkvm-design)
...
| Mnemonic | Operands (asm) | Encoded  | Description                                                                                                                                                                                                                                                                                                                                                                  |
| ----------- | --- | ---------------------------------------------------- | ----- |
...
| Mnemonic | <div style="width:170px">Operands (asm)</div> | <div style="width:170px">Encoded</div>    | Description  |
| ----------- | ------------- | --| ----------------------------------------------------------------------------------- |
...
| ----------- | ------------- | ----------------------------------------------------------------------------------- |
| **LOAD8**     | `a(fp), b(fp)` | Follow the pointer stored at element offset $b$ from the frame pointer and write the byte value to element offset $a$. |
| **STORE8**     | `b(fp), c(fp)` | Write the byte value at element offset $c$ to the value pointed to at element offset $b$.|
| **STORE8I**     | `b(fp), c` | Write the immediate value $c$ to the value pointed to at element offset $b$.|
...
Note that:
* Functions arguments are stored at fp + 4, fp + 8, ...
* Return address is stored at fp
* Return value is stored at fp - 4
* Local variables are stored at fp - 8, fp - 12, ...
* If a return value is not present, then local variables begin at fp - 4.
...
Our VM has general purpose registers, since memory is cheap.
...
_If you’re interested in further discussions on this topic or working together on this subject, please consider joining our [group chat](https://t.me/+9WAAmCpPRadjOTNh) or reach out to us at hello@delendum.xyz._